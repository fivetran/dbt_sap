...
| ---------- | ----------- | -------- | -------- | ----- |
| **All `stg_sap__*` staging models with a `_fivetran_deleted` column** (breaking) | Changed field/model | Soft-deleted source rows were included in staging output, and in any downstream aggregations (for example `mchb.clabs`, `marc`'s stock fields) | Soft-deleted rows (`_fivetran_deleted = true`) are now excluded from every staging model | Fixes incorrect data — for example negative `CLABS` values in `mchb` — caused by soft-deleted rows flowing into staging and the aggregations built on top of it. Row counts will decrease for any staging model whose source table has soft-deleted records, and everything built on these models (compatibility views, star schema, extractor reports) will reflect only active records going forward. |

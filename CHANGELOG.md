...
| ---------- | ----------- | -------- | -------- | ----- |
| **All `stg_sap__*` staging models with a `_fivetran_deleted` column** (breaking) | Changed field/model | Soft-deleted rows were included | Soft-deleted rows are excluded | Removes deleted rows in staging and downstream so aggregations (for example `mchb.clabs`) are calculated from active records only. |
